...
export const fetchShoppingCartItems = () => dispatch => {
  return ShoppingCartItemAPIUtil.fetchShoppingCartItems().then(
    shoppingCartItems => dispatch(recieveShoppingCartItems(shoppingCartItems))
  );
};

export const createShoppingCartItem = shoppingCartItem => dispatch => {
  return ShoppingCartItemAPIUtil.createShoppingCartItem(shoppingCartItem).then(
    shoppingCartItem => {
       return dispatch(recieveShoppingCartItem(shoppingCartItem))}
  );
};

export const deleteShoppingCartItem = id => dispatch => {
  return ShoppingCartItemAPIUtil.deleteShoppingCartItem(id).then(
    shoppingCartItemId => dispatch(removeShoppingCartItem(shoppingCartItemId))
  );
};
